...
router.get("/", async (req, res) => {
  ...
  if (select_category) {
    ...
    totalAmount = await Record.sum("amount", {
      where: { userId, categoryId: select_category },
    });
  } else {
    records = await Record.findAll({
      attributes: [
    ...
    totalAmount = await Record.sum("amount", { where: { userId } }).catch(
      (error) => {
        console.log(error);
        return res.redirect("back");
      }
...
router.post("/create", async (req, res) => {
  ...
  await Record.create({ name, date, categoryId, amount, userId }).catch(
    (error) => {
      console.log(error);
      return res.redirect("back");
    }
...
router.put("/:id/edit", async (req, res) => {
  ...
  const { name, date, categoryId, amount } = req.body;
  const userId = req.user.id;
  const record = await Record.findByPk(id).catch((error) => {
    console.log(error);
    return res.redirect("back");
  });
  if (!record) {
